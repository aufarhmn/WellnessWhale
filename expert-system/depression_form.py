...
def chat():
    global current_question_index, depression_score, user_responses
    
    user_input = request.json.get('message')
    
    if user_input.lower() == "sedih":
        current_question_index = 0
        user_responses = []
        depression_score = 0
        return jsonify({"response": depression_questions[current_question_index][0]})
    
    if current_question_index < len(depression_questions):
        ...
        # Hitung score
        depression_score += score * depression_questions[current_question_index][1]
        ...
        if current_question_index < len(depression_questions):
            ...
            if depression_score >= threshold:
                ...
                return jsonify({"response": "Sepertinya Anda tidak menunjukkan banyak gejala depresi, tapi tetap jaga kesehatan mental Anda!"})
    else:
